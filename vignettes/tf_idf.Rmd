...
The value for tf returned by `calculate_tf_idf` is not the same as what I get just using dplyr for many of the words.

```{r}
book_words %>%
  mutate(tf_dplyr = n/total, diff = tf - tf_dplyr) %>%
  select(-idf, -tf_idf)
```


There are some words where the two tf calculations agree:
...
Those are rare words, words with high tf-idf?!?!?
