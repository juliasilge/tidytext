...
As pre-processing, we divide these into chapters, use tidytext's `unnest_tokens` to separate them into words, then remove `stop_words`.^[Note that you [typically don't want to remove stop words before building topic models](http://dx.doi.org/10.1162/tacl_a_00099).] We're treating every chapter as a separate "document", each with a name like `Great Expectations_1` or `Pride and Prejudice_11`.
...
It seems like it is especially difficult to distinguish *Great Expectations* from *The War of the Worlds*.