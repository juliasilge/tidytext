...
## Using STM with Document-Level Covariates

The Structural Topic Model (STM) allows you to include document-level covariates that can affect topic prevalence. However, there's an important consideration when using tidy workflows: **the number of documents in your sparse matrix must match the number of rows in your covariate data**.

### The Problem: Stopword Removal Creates Mismatches

When you remove stop words, some documents may become empty and get dropped from your document-term matrix, causing a mismatch with your covariate data:
...
gadarian_sparse <- gadarian2 |>
  ...
  anti_join(stop_words) |>  # Some documents disappear here!
  count(document, word) |>
  cast_sparse(document, word, n)

# This will fail with a dimension mismatch error:
topic_model <- stm(
  gadarian_sparse,
  K = 3,
  prevalence = ~ treatment + s(pid_rep),
  data = gadarian2,  # Has more rows than gadarian_sparse!
  verbose = FALSE
)
```

### Solution 1: Keep All Words

The simplest solution is to keep all words (including stop words) so no documents are dropped:
...
### Solution 2: Filter Covariates to Match Documents

If removing stop words is important for your analysis, create a covariate dataframe that only includes documents present in the sparse matrix:

```{r stm_covariates_filtered}
gadarian2 <- gadarian |>
...
# Create filtered covariate data matching the sparse matrix
covariate_df <- tibble(document = rownames(gadarian_sparse)) |>
  inner_join(gadarian2, by = "document")
...
topic_model <- stm(
  ...
  init.type = "Spectral",
  prevalence = ~ treatment + s(pid_rep),
  data = covariate_df,  # Now the dimensions match!
  verbose = FALSE
)
...
This approach ensures your covariate data has exactly the same documents (and in the same order) as your document-term matrix.
