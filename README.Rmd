...
To work with this as a tidy dataset, we need to restructure it into a **one-token-per-row** format. The `unnest_tokens()` function is a way to convert a dataframe with a text column to be one-token-per-row:
...
#| fig.height = 8,
#| fig.width = 8,
#| fig.alt = "Scatterplot for word frequencies in Jane Austen vs. AP news articles. Some words like 'cried' are only common in Jane Austen, some words like 'national' are only common in AP articles, and some words like 'time' are common in both."
