...
bing_lexicon1 <- readr::read_lines("data-raw/positive-words.txt", skip = 35)
bing_lexicon2 <- readr::read_lines("data-raw/negative-words.txt", skip = 35)
...
parts_of_speech <- readr::read_csv(
  "Noun,N
                                   Plural,p
                                   Noun Phrase,h
...
# parts of speech
parts_of_speech <- readr::read_delim(
  "data-raw/mobyposi.i.zip",
  delim = "\xd7",
...
nma_words <- readr::read_lines("data-raw/list-English-negators.txt") |>
  ...
  bind_rows(
    readr::read_lines("data-raw/list-English-modals.txt") |>
      tibble(word = .) |>
      mutate(modifier = "modal")
  ) |>
  bind_rows(
    readr::read_lines("data-raw/list-English-adverbs.txt") |>
      tibble(word = .) |>
